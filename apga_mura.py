...
def mura_experiment(seed, body_part, n_runs, gpu_id, n_shot=None, train_cutout=False, train_apga=False,
                    train_gradcam=False, train_end2end=False):
    import warnings
        ...
        for epoch in range(n_epoch):  # loop over the dataset multiple times
            ...
            for i, data in tqdm(enumerate(train_generator)):
                ...
                if aug:
                    ...
                    aug_labels = Variable(torch.Tensor()).to(device=device, dtype=torch.long)
                    for j, img in enumerate(inputs):
                        aug_imgs = torch.cat([aug_imgs, mask_aug(policy, img, p=1, mask_action=True).unsqueeze(0)],
                                             dim=0)
                        aug_labels = torch.cat([aug_labels, labels[j].unsqueeze(0)], dim=0)
                    ...
                    if aug_imgs.dim() > 1:
                        ...
                        for k, img in enumerate(inputs):
                            # inputs[i] = mask_aug(policy, img, p=0.5)
                            # if labels[i] > 0:
                            aug_imgs = torch.cat([aug_imgs, mask_aug(policy, img, p=1, mask_action=False).unsqueeze(0)],
                                                 dim=0)
    ...
    def train_gradcam(model, gradcam_model, n_epoch, optimizer, loss_f, train_generator, val_generator, save_path,
                      gradcam=False, aug=False, patience=10):
        best_val_acc = 0
    ...
    def update_policy(policy, pg_optimizer, loss_reward, val_acc=None, images=None, sim_loss_fn=nn.BCELoss()):
        ...
        loss = ((sim_loss) * R) * 100 + zero_loss * torch.abs(R) * 0.1 + one_loss * torch.abs(
            R) * 0.01  # REINFORCE loss + penalty for masking too much
    ...
    for n in range(n_runs):
        ...
        class MURADataset(Dataset):
            ...
            def __init__(self,
                ...
                if not body_part:
                    ...
                    self.npy_paths = mura_frame['path'][mura_frame.path.apply(lambda s: body_part in s)].reset_index(
                        drop=True)
                    self.labels = self.npy_paths.apply(self.path_to_label).reset_index(drop=True)
                else:
                    for bodypart in multi_body_parts:
                        paths = mura_frame['path'][mura_frame.path.apply(lambda s: bodypart in s)].reset_index(
                            drop=True)
                ...
                if k_shot:
                    ...
                    self.pos_sample_indexes = np.random.choice(np.arange(len(self.positive_paths)), size=self.k_shot,
                                                               replace=False)
                    self.neg_sample_indexes = np.random.choice(np.arange(len(self.negative_paths)), size=self.k_shot,
                                                               replace=False)
            ...
            def set_data_distribution(self, p):
        ...
        elbow_train_dataloader = DataLoader(elbow_train_dataset, batch_size=batch_size, shuffle=True, num_workers=0)
        elbow_val_dataloader = DataLoader(elbow_val_dataset, batch_size=batch_size, shuffle=False, num_workers=0)
        ...
        train_loader_dict = {'elbow': elbow_train_dataloader, 'finger': finger_train_dataloader,
                             ...
                             'humerus': humerus_train_dataloader, 'shoulder': shoulder_train_dataloader,
                             'wrist': wrist_train_dataloader}
        ...
        val_loader_dict = {'elbow': elbow_val_dataloader, 'finger': finger_val_dataloader,
                           ...
                           'humerus': humerus_val_dataloader, 'shoulder': shoulder_val_dataloader,
                           'wrist': wrist_val_dataloader}
        ...
        baseline_acc, pg_acc, end2end_acc, gradcam_acc = 0, 0, 0, 0
        ...
        if train_end2end:
            ...
            end2end_acc = train_end2end(policy, model, 100, optimizer, pg_optimizer,
                                        ...
                                        end2end_save_path, policy_save_path, best_val_acc=0, cutout=False, patience=15,
                                        aug_reg=True)
        ...
        if train_gradcam:
            ...
            gradcam_acc = train_gradcam(model, gradcam_model, 100, optimizer, loss_f, train_dataloader, val_dataloader,
                                        gradcam_save_path,
                                        gradcam=True,
                                        aug=False, patience=15)
    ...
    def show_debug_mask_mura(idx, dataset, body_part='elbow', save=True):
        ...
        debug_mask, mask = predict_masks_from_imgs(policy,
                                                   ...
                                                   morphology_cleaning=False, largest_connected=False,
                                                   fill_enclosed=False)
        if save:
            mask = np.moveaxis(mask, -1, 0)
            show_and_save(debug_mask.numpy(),
                          'policy_gradient_pngs_mura/' + 'masked_' + body_part + '_' + str(i) + '_class_' + str(
                              label) + '.png')
            show_and_save((mask > 0.5),
                          'policy_gradient_pngs_mura/' + '_' + body_part + '_mask' + str(i) + '_class_' + str(
                              label) + '.png',
                          cmap=plt.get_cmap('gray'))
        else:
            plt.figure()
